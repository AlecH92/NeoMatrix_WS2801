...
NeoMatrix_WS2801::NeoMatrix_WS2801(int w, int h, uint8_t dataPin, uint8_t clockPin,
  uint8_t matrixType, int ledType) : Adafruit_GFX(w, h),
  Adafruit_WS2801(w, h, dataPin, clockPin, ledType), type(matrixType), matrixWidth(w),
  matrixHeight(h), tilesX(0), tilesY(0), remapFn(NULL) { }

// Constructor for tiled matrices:
NeoMatrix_WS2801::NeoMatrix_WS2801(uint8_t mW, uint8_t mH, uint8_t tX,
  uint8_t tY, uint8_t dataPin, uint8_t clockPin, uint8_t matrixType, int ledType) :
  Adafruit_GFX(mW * tX, mH * tY), Adafruit_WS2801(mW * mH * tX * tY, dataPin, clockPin,
...
uint16_t NeoMatrix_WS2801::Color(uint8_t r, uint8_t g, uint8_t b) {
...
void NeoMatrix_WS2801::setPassThruColor(uint32_t c) {
  passThruColor = c;
  passThruFlag  = true;
}

// Call without a value to reset (disable passthrough)
void NeoMatrix_WS2801::setPassThruColor(void) {
  passThruFlag = false;
}

void NeoMatrix_WS2801::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void NeoMatrix_WS2801::fillScreen(uint16_t color) {
  uint16_t i, n;
  uint32_t c;
...
void NeoMatrix_WS2801::setRemapFunction(uint16_t (*fn)(uint16_t, uint16_t)) {
  remapFn = fn;
}
