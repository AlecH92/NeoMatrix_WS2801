...
class NeoMatrix_WS2801 : public Adafruit_GFX, public Adafruit_WS2801 {

 public:

  // Constructor for single matrix:
  NeoMatrix_WS2801(int w, int h, uint8_t dataPin = 2, uint8_t clockPin = 3,
    uint8_t matrixType = NEO_MATRIX_TOP + NEO_MATRIX_LEFT + NEO_MATRIX_ROWS,
    int ledType = WS2801_RGB);

  // Constructor for tiled matrices:
  NeoMatrix_WS2801(uint8_t matrixW, uint8_t matrixH, uint8_t tX,
    uint8_t tY, uint8_t dataPin = 2, uint8_t clockPin = 3, 
    uint8_t matrixType = NEO_MATRIX_TOP + NEO_MATRIX_LEFT + NEO_MATRIX_ROWS +
